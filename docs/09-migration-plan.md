...
This document defines the intended scope for the first Supabase/PostgreSQL migration before any SQL is written.
...
Owner approval is required before any SQL, migration files, database functions, triggers, or RLS policies are created.

## 2. Purpose

The purpose of this migration plan is to turn the approved MVP data model and RLS planning decisions into a clear first-migration scope.
...
- Conceptual support for database-backed receipt number sequencing.
- Initial RLS policy groups for organization isolation.
...
- Database enforcement for invoice total consistency may be considered later but is not required for initial migrations.
...
- Receipt number sequencing should be database-backed conceptually.
...
The migration implementation should plan automatic `updated_at` maintenance. The exact implementation must be approved before SQL is written.
...
The first migration should conceptually plan indexes for RLS, relationships, common listing screens, dashboards, and uniqueness.

Likely important indexes:

- `profiles.organization_id`.
- `properties.organization_id`.
- `properties (organization_id, name)` for listing or search.
...
- `tenants (organization_id, full_name)` for listing or search.
- `tenants (organization_id, phone)` for phone lookup.
...
- `invoice_line_items (invoice_id, sort_order)` for display order.
- `payments.organization_id`.
- `payments.invoice_id`.
- `payments (organization_id, payment_date)` for reporting.
- `payments (organization_id, payment_method)` for filtering if useful.
...
- `maintenance_tickets (organization_id, priority)` for filtering if useful.
- `reminders.organization_id`.
- `reminders.invoice_id`.
- `reminders.tenant_id`.
- `reminders (organization_id, status)`.
- `reminders (organization_id, channel)` for filtering if useful.

The final migration implementation should choose the smallest useful index set. Optional indexes can be deferred if they do not support immediate MVP reads, constraints, or dashboard queries.
...
4. The exact database-backed mechanism must be designed and approved before SQL is written.
5. Receipt creation should remain simple structured data, not a complex accounting or tax-invoice workflow.

Open implementation decision before SQL:

- Choose the exact database-backed receipt sequencing mechanism.

Acceptable future migration-planning options may include a dedicated sequence-tracking structure or a transactional database function, but this document intentionally does not define SQL or implementation details.

## 9. RLS Policy Planning

RLS should enforce the organization boundary in the database.
...
- Supabase migration files before owner approval.
- SQL files before owner approval.
- SQL policy definitions in this planning document.
- Database function or trigger SQL in this planning document.
- Source code.
...
- Database enforcement for invoice total consistency in the first migration.
- Database checks or triggers for cross-organization relationship protection in the first migration.

## 11. Migration Readiness Checklist

Before any SQL is written, confirm:

- [ ] Owner has reviewed this migration plan.
- [ ] Owner has approved the exact first migration scope.
- [ ] Owner has approved the final table list.
- [ ] Owner has approved the planned constrained values.
- [ ] Owner has approved the primary key and foreign key direction.
- [ ] Owner has approved `organization_id` usage.
- [ ] Owner has approved timestamp and `updated_at` maintenance direction.
- [ ] Owner has approved `cancelled_at` usage only on `leases`, `invoices`, `maintenance_tickets`, and `reminders`.
- [ ] Owner has approved planned unique constraints.
- [ ] Owner has approved the initial index set, including which optional indexes can be deferred.
- [ ] Owner has approved the exact database-backed receipt sequencing mechanism.
- [ ] Owner has approved the exact RLS policy implementation plan.
- [ ] Owner has confirmed that same-organization relationship protection can start with application validation plus normal foreign keys.
- [ ] Owner has confirmed that database checks or triggers for cross-organization relationship protection are deferred.
- [ ] Owner has confirmed invoice total consistency starts with application validation.
- [ ] Owner has confirmed database enforcement for invoice total consistency is not required for the first migration.
- [ ] Owner has confirmed no out-of-scope product tables are being added.
- [ ] A separate implementation task has been created for SQL and migration files after approval.

## 12. Owner Approval Required Before SQL

Owner approval is required before creating:
...
Until owner approval is given, database work must remain planning documentation only.
